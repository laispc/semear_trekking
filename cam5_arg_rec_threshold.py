# Teste version
# No serial writing
# Usage: argument argv[1] is the camera index you want to use
# The video capture is saved as 'output'+cam_number+'.avi'
...
#cap = cv2.VideoCapture('output_rocobore.avi')
print "camera:" + repr(cam_number)
...
out = cv2.VideoWriter('output'+cam_number+'.avi',fourcc, 10, (640,npixel)) # w,h
...
while(cap.isOpened()):
	...
	'''#COMMENT THIS BLOCK FOR FRAME BY FRAME ANALYSIS
	k = cv2.waitKey()

	while (k != ord('p') and k != ord('q')):
		k = cv2.waitKey()
	'''
